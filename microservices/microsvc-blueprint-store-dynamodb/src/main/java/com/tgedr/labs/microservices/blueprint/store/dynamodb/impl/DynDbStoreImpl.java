...
class DynDbStoreImpl implements Store {
	...
	public Optional<Task> getTask(final String taskId) throws StoreException {
		...
		try {
			Optional<com.tgedr.labs.microservices.blueprint.store.dynamodb.entities.Task> r = repository.findById(taskId);
			if( r.isPresent() )
	...
	public Set<Task> getTasks() throws StoreException {
		...
		try {
			for(com.tgedr.labs.microservices.blueprint.store.dynamodb.entities.Task t:repository.findAll())
